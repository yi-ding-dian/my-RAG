...
## 与主项目（my-RAG 知识库系统）的关系

本组件是 **my-RAG 套件**的配套评估组件（仓库根目录 `README.md` 对本仓库有总体介绍）：

- 主项目（RAG 知识库系统，`../RAG/`）通过 **HTTP（端口 8090）** 对接本系统，接口为
  **只读 / 评估发起**性质：知识库侧可读取评估任务列表与报告（`/api/stats/ragas`），
  也可发起评估（`POST /api/stats/ragas/evaluations`，由知识库侧采样真实问答并填充检索上下文）；
- 在知识库管理端「**统计分析**」页可一键发起 RAGAS 评估并查看结果；
- 主项目未启动 / 本系统未运行时，知识库侧自动降级（页面提示，不影响自身统计）。
- 两系统共用同一套 LLM / Embedding 基础设施；部署方式见 `RAG/docs/external-deps.md`。
