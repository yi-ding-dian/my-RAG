...
- 文档上传（txt/md/pdf/docx/**xlsx/xls/csv**，≤100MB）+ **URL 网页导入** + 中文文件名安全（内部 UUID 命名）；Excel/CSV 结构化直读为管道表格入库
- 解析方式可选：MinerU 高质量解析（含图片提取）/ 纯文本降级；切块方式：通用/按标题/正则/父子分块/QA 问答/Agentic 智能分块（LLM 语义切块+类型标签，≤1 万字，与上下文检索/知识图谱互斥）
