...
- hybrid_levels：**混合方案**——带编号的标题按编号规则确定性定级（零成本），
  ...
  实测全量判定数百个标题要近 2 万 token / 4 分钟左右，混合只用 1/4 左右的
...
# 关键词只是启发式（实测缺陷：真章节标题「1.1.1 注意事项」被"注意事项"
...
# （实测这类行绝大多数被 LLM 判 0），故不参与规则定级、交 LLM 判
...
def context_label(headings: List[Tuple[int, int, str]],
    ...
    - style=_CTX_PREV：前文最近的带编号标题及其层级（省 token，但中间隔着
      无编号标题时会指偏——实测：层级低的"1、xxx"会被前面层级更高的编号
      带偏、判成了正文行）；
