...
- GET /{doc_id}/docx-outline  Word 文档标题结构预览（「查看文档结构」弹窗数据源：
  结构解析会把文档解析成什么样的标题层级树，doc 经 LibreOffice 转换）
...
async def ingest_document(request: Request, kb_id: str, doc_id: str,
    ...
    if ing_svc.is_running(doc_id) or ing_svc.is_queued(doc_id):
...
async def get_document_raw(kb_id: str, doc_id: str,
    ...
    - doc：同 docx（老二进制 Word 不预览，一律附件下载）
    - 其他类型暂不支持（400）
    - 回收站文档不可预览（404 伪装）
    """
    await kb_or_404(db, kb_id, user)
    ...
    if file_type not in ("pdf", "txt", "md", "url", "docx", "doc",
                         "xlsx", "xls", "csv"):
    ...
    if file_type in ("docx", "doc"):
        ...
        # doc（老二进制 Word）不支持在线预览，与 docx 同走下载分支
    ...
    # 与 WPS 打开观感一致（原理见 services/spreadsheet/preview.py）
    if file_type in ("xlsx", "xls", "csv"):
        try:
            from backend.services.spreadsheet.preview import \
...
_DOWNLOAD_MEDIA_TYPES = {
    ...
    "doc": "application/msword",  # 老二进制 Word
...
def _outline_failure_detail(exc: Exception) -> str:
    """结构提取失败的中文可读原因（损坏/加密/非 OOXML 不给裸英文异常）

    非 OOXML/损坏/加密的 Word 文件在 python-docx 打开阶段就抛 PackageNotFound
    （不是 zip 包）/ BadZipFile（zip 结构损坏）/ KeyError（部件缺失）；
    LibreOffice 转换失败是 RuntimeError，其 message 本就是中文，直接用。
    """
    from docx.opc.exceptions import PackageNotFoundError
    if isinstance(exc, (PackageNotFoundError, BadZipFile, KeyError)):
        return "文件不是有效的 Word 文档（可能已损坏、加密或非 OOXML 格式）"
    return str(exc) or exc.__class__.__name__


def _extract_outline_sync(path: Path,
                          file_type: str) -> Tuple[List[dict], Optional[str]]:
    """标题结构提取的同步部分（线程池执行，见 get_docx_outline 的 to_thread）

    - docx 直接读；doc 先经 LibreOffice 转 docx（convert_doc_to_docx），
      临时目录在 finally 清理（转换产物与独立 profile 都在其中，见该函数
      的调用约定）；
    - 返回 (items, warning)：提取失败返回空列表 + 中文原因——不向上抛异常
      （接口恒 200，前端按 warning 提示，与画像接口的容错口径一致）。
    """
    from backend.normative import extract_outline
    from backend.services.parsers.client import convert_doc_to_docx
    tmp_dir = None
    try:
        if file_type == "doc":
            path, tmp_dir = convert_doc_to_docx(path)
        return extract_outline(path), None
    except Exception as e:
        logger.warning("文档结构提取失败 %s: %s", path.name, str(e)[:200])
        return [], _outline_failure_detail(e)
    finally:
        if tmp_dir is not None:
            shutil.rmtree(tmp_dir, ignore_errors=True)


@router.get("/{doc_id}/docx-outline", response_model=DocxOutlineResponse)
async def get_docx_outline(kb_id: str, doc_id: str,
                           db: AsyncSession = Depends(get_db),
                           user: UserPublic = Depends(get_current_user)):
    """文档标题结构预览（can_access_kb；「查看文档结构」弹窗数据源）

    - 仅 Word 文档（docx/doc）有效：预览结构解析（docx_struct）会把这份
      文档解析成什么样的标题层级树（层级 + 标题文本，自动编号已还原），
      供用户在解析前确认；其他文件类型 400；
    - doc：先经 LibreOffice 转 docx 再提取（临时目录即用即清，见
      _extract_outline_sync）；
    - 提取失败（损坏/加密/非 OOXML/转换失败）→ 200 + warning 中文原因
      （items 为空），前端用 Alert 提示，不抛 500 裸异常；
    - 同步阻塞（尤其 doc 转换）放线程池执行，不阻塞事件循环；
    - 回收站文档不可查看（404 伪装）。
    """
    await kb_or_404(db, kb_id, user)
    doc = _get_doc_or_404(kb_id, doc_id)
    if doc.deleted:
        raise HTTPException(status_code=404, detail="文档不存在")
    file_type = (doc.file_type or "").lower().lstrip(".")
    if file_type not in _WORD_FILE_TYPES:
        raise HTTPException(
            status_code=400,
            detail="仅 Word 文档（docx/doc）支持查看文档结构")
    upload_path = get_document_service().get_upload_path(doc)
    if not upload_path.exists():
        raise HTTPException(status_code=404, detail="文件不存在或已被清理")
    items, warning = await asyncio.to_thread(
        _extract_outline_sync, upload_path, file_type)
    return DocxOutlineResponse(
        items=[DocxOutlineItem(**item) for item in items],
        count=len(items),
        max_level=max((int(item["level"]) for item in items), default=0),
        warning=warning)
...
async def purge_document(request: Request, kb_id: str, doc_id: str,
    ...
    await kb_or_404(db, kb_id, user, manage=True)
    doc = _get_doc_or_404(kb_id, doc_id)
    # 仅回收站内文档可彻底删除：回收站的意义是"先软删、确认后再物理删"，
    # 漏掉这道校验会让调用方（含脚本直连/误调用）绕过回收站做不可恢复的
    # 删除（存储对象 + 向量 + 元数据 + 图谱引用一并清掉）
    if not doc.deleted:
        raise HTTPException(status_code=409,
                            detail="文档不在回收站，请先移入回收站再彻底删除")
