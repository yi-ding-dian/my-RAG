"""超管全局文档管理 API：/api/admin/documents（仅 super_admin）
...
- 删除/重命名不在此模块重复实现：前端直接调现有
  ...
  can_manage_kb 对 super_admin 恒放行（deps.can_manage_kb 首行返回 True），
  超管天然具备全部门管理权限，无需新接口。
...
权限：require_super_admin（user/dept_admin → 403；与部门内文档列表的
404 伪装策略不同——本接口是超管专属管理面，不暴露存在性信息价值）。
"""
...
async def list_admin_documents(
    ...
    """跨部门全部文档查询（仅 super_admin）

    响应契约: {total, page, page_size, items: [文档字段 + kb_name,
    department_id, department_name]}；先过滤后分页，total 为过滤后数量
    （语义与部门内文档列表一致，避免"筛选只作用于当前页"的误导）。
    默认按创建时间倒序（与部门内列表一致）。
    """
    if status:
