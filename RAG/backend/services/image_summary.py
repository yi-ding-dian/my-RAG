...
一行 `![](images/xxx.jpg)`，于是"图里的权属人是谁"这类问题**永远命中不了**
那张图。本模块让多模态模型把图读出来、写成一段描述回填 markdown，
从而参与切块与检索。
...
# 用相对判定而非绝对阈值的原因：实测同一份文档里大幅扫描图可达百万像素级、
# 小印章图只有几万像素，绝对阈值（如 4 万）根本挡不住；相对判定才有效。
# 绝对下限兜底"整篇都是小图"的文档，避免全跳。阈值刻意保守——误跳一张有用
# 的小图（签名、二维码）比省一次调用代价大得多。
_SMALL_RATIO = 0.15
_MIN_PIXELS = 200 * 200
