...
SECTION_SCHEMA: Dict[str, SectionSpec] = {
    ...
    # heading_llm_model=标题分层模型（可选，层级聚合切块用，值为 LLM 模型
    # 列表标识）：on_null=restore 允许清空（恢复"不做 LLM 分层"的默认语义）；
    # 不加 fill_missing——保持 coerce 对旧档案不补该字段的既有契约
    ...
    # 信号量按配置值惰性重建，见 backend/services/ingestion/service.py 的 _get_ingest_semaphore）；
