...
- 连接测试：LLM 发最小 chat 请求、Embedding 发 1 条 embed、MinerU 健康探测、
  ...
  MinIO bucket 探测 → 返回 {ok, latency_ms, message} 结构（实现见 settings.connect_test）
- 密钥字段脱敏泛化（is_secret_field: endswith api_key/password/secret_key）：
  GET 返回脱敏；保存时传回脱敏值不覆盖原值

======== 按职责拆分（本文件为编排层，纯逻辑按职责下沉到同目录模块） ========
- settings/schema.py：SECTION_SCHEMA 声明式反射 / 字段白名单 / 密钥与类型辅助 /
  默认值构造 / coerce（类型规范化 + 旧档案兼容）——**新增配置段改这里**
  （维护者指南见该文件 docstring）
- settings/merge.py：chat_payload / merge_chat_config / merge_department_llm /
  active_llm_item 等纯合并函数
- settings/validate.py：_FIELD_LABELS / validate_range（update_profile 范围校验）
- settings/connect_test.py：SettingsTester mixin（连接测试，含 LLM_TEST_TIMEOUT 等常量）
- 本文件保留对外主入口 / 档案 CRUD / 活跃配置获取 / 合并入口（re-export）/
  依赖单例的查询（find_llm_item / llm_cfg_for_parser）——全部原符号照常
  可从 backend.services.settings.service import（routers/chat_service/
...
from backend.services.settings.merge import (active_llm_item, chat_payload,
                                             merge_chat_config,
...
from backend.services.settings.connect_test import (DEEPDOC_TEST_TIMEOUT,
                                            ...
                                            MYSQL_TEST_TIMEOUT,
                                            SettingsTester)
from backend.services.settings.validate import _FIELD_LABELS, validate_range
...
# ================= 依赖单例的查询（连接测试/合并函数见 settings.connect_test / settings.merge） =================
