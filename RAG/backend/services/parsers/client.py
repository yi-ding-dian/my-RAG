"""文档解析客户端（解析引擎可选：MinerU 高精度 / 纯文本提取 / 结构解析 / 自动）
...
- parse(file_path, file_type, engine="auto", **parse_opts):
  ...
  - engine="deepdoc": 强制走 DeepDoc（RAGFlow API，见 parsers/deepdoc.py），
    表格输出为 HTML <table> 可检索；仅 PDF 支持（docx 抛异常）；不可用或
    失败抛异常（上层标记 failed，错误提示指向 DeepDoc 服务地址）
  - engine="docx_struct": 强制走本地结构化解析（backend.normative，
    OOXML 直读：标题层级/自动编号/表格/图片）；仅 docx/doc 支持，
    纯本地无服务依赖，失败抛异常（上层标记 failed）；.doc 老二进制格式
    经 LibreOffice 转 docx 后解析（见 convert_doc_to_docx）
...
from backend.services.parsers.probes import probe_mineru
...
# ---------- 老二进制 .doc 转换（LibreOffice headless） ----------

# LibreOffice 转换超时（秒）：冷启动/大文档较慢，留足余量；超时按失败处理
_DOC_CONVERT_TIMEOUT = 300


def convert_doc_to_docx(path: Path,
                        timeout: float = _DOC_CONVERT_TIMEOUT
                        ) -> Tuple[Path, Path]:
    """老二进制 .doc → .docx（LibreOffice headless），返回 (docx 路径, 临时目录)

    - 独立用户 profile（-env:UserInstallation=file://<临时目录>/profile）：
      避免与用户机器上已打开的办公软件抢占同一 profile 锁而挂起/失败；
    - 转换产物落在临时目录（含 profile），**调用方用完负责清理整个临时目录**
      （shutil.rmtree(tmp_dir, ignore_errors=True)）；
    - 超时（_DOC_CONVERT_TIMEOUT 秒）/ 未安装 soffice / 未产出 docx（文件
      损坏、加密、非 Word 二进制）→ RuntimeError（中文提示，上层标记 failed）。
    """
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "未找到 LibreOffice（soffice），无法解析 .doc 文档，"
                "请转存为 docx 后上传") from e
        except subprocess.TimeoutExpired as e:
            raise RuntimeError(
                f".doc 转换超时（超过 {int(timeout)} 秒），"
                f"文件可能过大或已损坏，请转存为 docx 后上传") from e
        # 输出文件名跟随输入名（中文安全）：用 glob 兜底取实际产物
        outputs = list(tmp_dir.glob("*.docx"))
        if proc.returncode != 0 or not outputs:
            detail = (proc.stderr or b"").decode("utf-8", "replace").strip()
            raise RuntimeError(
                ".doc 转换失败，文件可能已损坏或被加密，"
                "请用 Word/WPS 另存为 docx 后上传"
                + (f"（{detail[:200]}）" if detail else ""))
        logger.info("doc 转 docx 完成: %s（耗时 %.1fs）", path.name,
                    time.time() - t0)
        return outputs[0], tmp_dir
    except Exception:
        shutil.rmtree(tmp_dir, ignore_errors=True)
        raise
...
class ParserClient:
    ...
    async def _mineru_available(self, api_url: str) -> bool:
        """MinerU 健康探测（复用 parsers.probes 统一探测：<500 可用，3s 超时）"""
    ...
    async def _parse_via_deepdoc_forced(self, file_path: Path,
        ...
        from backend.services.parsers.deepdoc import parse_via_deepdoc
    ...
    # ---------- 本地结构化解析（engine=docx_struct） ----------

    async def _parse_via_docx_struct_forced(
            self, file_path: Path,
            file_type: str) -> Tuple[str, List[dict], str]:
        """强制本地结构化解析：OOXML 直读（backend.normative.parse_docx）

        - docx：直接解析（标题层级用 # 数量表示、自动编号已还原、表格为管道
          表格、图片引用为 images/{name}，与 MinerU 产物同构）；
        - doc：LibreOffice headless 转 docx 后同上（临时目录即用即清，
          见 convert_doc_to_docx）；
        - 纯本地实现（无服务依赖）：失败抛 RuntimeError（中文提示文件损坏/
          加密/转换超时），上层标记 failed
        """
        if file_type not in ("docx", "doc"):
            raise RuntimeError(
                "结构化解析仅支持 docx/doc 文档（其他类型请改用其他引擎）")
        from backend.normative import parse_docx
        tmp_dir: Optional[Path] = None
        try:
            if file_type == "doc":
                # 转换是阻塞式外部进程（大文件约数秒~数十秒）：放线程池执行，
                # 避免阻塞事件循环（其他请求/心跳不受影响）
                file_path, tmp_dir = await asyncio.to_thread(
                    convert_doc_to_docx, file_path)
            try:
                # 解析为纯 CPU 本地处理（大文档耗时明显），同样放线程池
                text, images, _method = await asyncio.to_thread(
                    parse_docx, file_path)
            except Exception as e:
                raise RuntimeError(
                    f"结构化解析失败: {e}，"
                    f"请确认文件为完整的 Word 文档（损坏/加密文件无法解析）"
                ) from e
        finally:
        ...
        if not text.strip():
            raise RuntimeError(
                "结构化解析结果为空（文档无正文内容），请改用其他引擎")
        return text, images, "normative"
    ...
    async def parse(self, file_path: Path, file_type: str,
        ...
        if file_type in ("xlsx", "xls", "csv"):
            from backend.services.spreadsheet.reader import (
        ...
        # ---- 老二进制 .doc：本地结构化解析（LibreOffice 转 docx 后直读）----
        # MinerU 不支持老二进制格式、纯文本提取（python-docx）也读不了，
        # doc 只有本地结构化解析一条路径：显式 docx_struct 或 auto
        # （auto 语义=自动选择可用解析方式，对 doc 即本地结构化解析）
        if file_type == "doc":
            if engine in ("docx_struct", "auto"):
                return await self._parse_via_docx_struct_forced(
                    file_path, file_type)
            raise RuntimeError(
                f".doc 文档仅支持结构化解析（docx_struct/auto），"
                f"当前引擎 {engine} 不支持；"
                f"请改用结构化解析，或转存为 docx 后重新上传")

        if file_type in ("pdf", "docx"):
            cfg = get_active_config().mineru
            if engine == "docx_struct":
                # 本地结构化解析（OOXML 直读：标题层级/自动编号/表格/图片）；
                # 无外部服务依赖，失败直接抛异常（无降级链，本地解析无"服务
                # 不可用"概念，失败即文件问题）
                return await self._parse_via_docx_struct_forced(
                    file_path, file_type)
            if engine == "deepdoc":
