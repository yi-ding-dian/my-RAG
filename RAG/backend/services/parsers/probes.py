...
stats precheck / parsers.probe 的 {available, reason}）。
...
- 形态说明（历史契约兼容，行为零变化）：
  - 异步形态 probe_mineru/probe_deepdoc（httpx.AsyncClient）：parsers.probe、
    parsers.client 使用（其测试 mock httpx.AsyncClient）；
  - 同步形态 probe_mineru_sync/probe_deepdoc_sync（httpx.get/post）：
    settings.service 连接测试使用（其测试 mock httpx.get/post）；
...
- 循环导入规避：依赖的服务模块（parsers.deepdoc/aiomysql/minio）在函数内
  局部导入。
"""
...
# MinerU 官方 API 服务可能的健康端点（按序探测；与历史 parsers.client 端点一致）
...
async def probe_mineru(cfg=None, *, timeout: Optional[float] = None,
    ...
    """MinerU 健康探测（异步形态，httpx.AsyncClient；parsers.probe/parsers.client 用）

    - 端点：/health → /api/health → 根路径，任一响应 < ok_under 即可用
      （默认 <500：非服务器错误即视为服务在，与 parsers.client 历史语义一致）
...
def probe_mineru_sync(cfg=None, *, timeout: Optional[float] = None,
    ...
    """MinerU 健康探测（同步形态，httpx.get；settings.service 连接测试用）
...
def _deepdoc_login_payload(cfg) -> dict:
    from backend.services.parsers.deepdoc import _rsa_encrypt_password
...
async def probe_deepdoc(cfg=None, *, timeout: Optional[float] = None) -> dict:
    """DeepDoc 登录探测（异步形态，httpx.AsyncClient；parsers.probe 用）

    与 parsers.deepdoc 实际解析登录同契约（RSA 加密密码 + 响应头 token），
...
def probe_deepdoc_sync(cfg=None, *, timeout: Optional[float] = None) -> dict:
    """DeepDoc 登录探测（同步形态，httpx.post；settings.service 连接测试用）

    注意：settings.service.test_connections 在 async 上下文调用，本函数为
...
def probe_rerank_sync(cfg=None, *, timeout: Optional[float] = None) -> dict:
    ...
    - 同步形态，httpx.post；settings.service 连接测试用（async 上下文直接调用）
