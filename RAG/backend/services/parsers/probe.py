...
- probe_parsers(cfg=None, *, mineru_timeout, deepdoc_timeout) -> dict：
  ...
  - mineru：GET {url}/health → /api/health → 根路径（与 parsers.client
    探测端点一致），任一响应 <500 即可用
  - deepdoc：复用 parsers.deepdoc 登录探测（RSA 加密密码 POST
...
- 超时：mineru 与 deepdoc 并行探测（asyncio.gather），总耗时 = 两者最大
  ...
  传 3.0/5.0（解析前检测 ≤5s）
- 探测实现统一在 services/parsers/probes.py（probe_mineru/probe_deepdoc），本模块
