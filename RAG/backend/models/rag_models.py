...
class IngestRequest(BaseModel):
    ...
    image_summary: Optional[bool] = Field(None, description="图片摘要：开启后用「图片解析模型」（系统配置里配的多模态模型，部门可选用哪个）把图片内容读成一段描述、回填到图片下方使其可被检索——证照/扫描件里的文字因此第一次能搜到（产生额外 token 费用；单图失败/未配置模型都跳过不阻塞入库；默认 False）")
    image_summary_format: Optional[str] = Field(None, description="图片摘要输出格式（仅 image_summary=true 时生效，覆盖本次入库的格式）：fields=固定字段（检索命中率最高）/prose=段落描述/brief=一句话简介（适合整页文字但无需理解含义的图）；缺省=用部门或全局配置的格式。选了与部门配置**不同**的格式时，提示词改用该格式的内置模板——部门自定义的那份是照旧格式写的，硬套会让模型按旧格式作答、而代码按新格式解析；**部门配置不受影响**，本次选择也不持久化（重跑需重新选）")
