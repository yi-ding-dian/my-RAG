...
const ParseConfigModal: React.FC<ParseConfigModalProps> = ({ open, doc, kbId, onCancel, onSuccess }) => {
  ...
  // 「查看文档结构」弹窗开关（解析方式=结构解析时可打开，见下方链接文字）
  ...
  // backend/services/ingestion/service.py 的 _ingest）；txt/md/url 文档无版面识别/页码等链路，隐藏。
  // 隐藏时相关字段不渲染、提交时剔除（见 handleOk），后端缺省默认兜底。
  const docFileType = (doc?.file_type ?? '').toLowerCase().replace(/^\./, '');
  // doc 与 pdf/docx 一样需走解析器（doc 由后端转换后结构化解析），解析配置区同样显示/提交
  const isPdfLike = docFileType === 'pdf' || docFileType === 'docx' || docFileType === 'doc';
  // 结构化解析仅对 Word 系文档有意义（OOXML 直读，读的是标题样式/自动编号）
  const isWordLike = docFileType === 'docx' || docFileType === 'doc';
  ...
  useEffect(() => {
    if (open && doc) {
      ...
      if (cfg.layout_recognize === 'DocxStruct') parseMode = 'DocxStruct';
      else if (cfg.layout_recognize === 'DeepDOC') parseMode = 'DeepDOC';
      else if (cfg.layout_recognize === 'PlainText') parseMode = 'PlainText';
      else if (oldEngine === 'docx_struct') parseMode = 'DocxStruct';
  ...
  // 层级聚合切块可选性：所有走解析器的文档（pdf/docx/doc）都开放
  // - Word + 结构解析：标题层级由 OOXML 直读，精确，直接可用；
  // - PDF 等 OCR 类产物：层级常丢失（输出全 ## 扁平结构），此时由后端用
  //   系统配置的「标题分层模型」重新分层后再聚合；**未配置该模型时后端
  //   回退规则分层并在日志留痕**（不阻塞入库），效果有限，选项描述里给出提示
  const isHierarchicalAvailable = isPdfLike;

  // 解析方式 ↔ 切块方式联动（仅响应用户手动切换解析方式；打开弹窗回填、
  // 解析器不可用时自动切换都不会触发）：切到结构解析且切块方式仍是默认值
  // （naive）时自动改用层级聚合切块；用户显式选过的其他切块方式（如父子
  // 分块）不覆盖。
  // 从结构解析切走时**不再回退**：层级聚合切块对 PDF 等同样可选（后端用
  // 「标题分层模型」做 LLM 分层），选项不会消失，保留用户的选择
  const handleParseModeChange = (next: ParseMode) => {
    const current = (form.getFieldValue('method') as ParseMethod | undefined) ?? 'naive';
    if (next === 'DocxStruct' && current === 'naive') {
      form.setFieldValue('method', 'hierarchical');
    }
  };
  ...
  const handleOk = async () => {
    ...
    // 对应引擎（前端无自动档：MinerU→mineru / DeepDOC→deepdoc /
    // DocxStruct→docx_struct / PlainText→plain）
    if (isPdfLike) {
      config.parser_engine =
        values.parse_mode === 'MinerU' ? 'mineru'
          : values.parse_mode === 'DeepDOC' ? 'deepdoc'
            : values.parse_mode === 'DocxStruct' ? 'docx_struct' : 'plain';
      // 结构解析没有"版面识别"档位（parser_engine 已显式指定引擎），不提交该字段
    ...
    if (isPdfLike) {
      ...
      } else if (chosenEngine === 'mineru' && parserStatus?.mineru && !parserStatus.mineru.available) {
        // 本地解析（plain/docx_struct）不依赖外部服务，无需探测提示
        message.warning(
  ...
  // 不阻断选择）；纯文本本地直提恒可用（绿√）
  // 结构解析选项：仅 Word 文档（docx/doc）有意义——其他格式没有可读的标题样式。
  // 本地解析无外部服务依赖，恒可用（不参与探测）
  const docxStructOptions: Array<{ value: ParseMode; label: React.ReactNode; disabled?: boolean }> =
    isWordLike
      ? [{
          value: 'DocxStruct',
          label: (
            <ParseModeOptionLabel
              label="DOCX 结构解析（保留标题层级与自动编号，仅 Word 文档）"
              entry={{ available: true, reason: '' }}
              probing={false}
            />
          ),
        }]
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" style={{ marginTop: 8 }}>
            ...
            items={[
              {
                ...
                children: (
                  <>
                    ...
                    {/* 结构解析：选项下方给"查看文档结构"入口（与画像「引擎
                        建议：结构解析」标签同一弹窗）——解析前预览这份文档会被
                        解析成什么样的标题层级树 */}
                    {parseMode === 'DocxStruct' && isWordLike && (
                ...
                <Card title="子块配置" size="small" style={{ marginBottom: 16 }}>
                  <Form.Item
                    ...
                    rules={[
                      ...
                      // D3：子块上限与后端校验统一为 20000（backend/services/
                      // ingestion/params.py 的 _MAX_CHUNK_SIZE，naive/title/regex 档同值；原 2000 与后端
        ...
        {/* 层级聚合切块提示：该方式的块首自带"祖先 > 父 > 自己"标题链（由章节树
            直接生成），后端对它跳过标题链后处理（开关开了也不叠加），提示用户
            无需再开上面的「包含父标题」 */}
        {method === 'hierarchical' && (
          <Alert
            message="层级聚合切块块首自带标题链（祖先 > 父 > 自己），无需再开「包含父标题」"
            type="info"
            showIcon
            style={{ marginBottom: 16 }}
          />
    ...
    {/* 「查看文档结构」弹窗（解析方式=结构解析时由选项下方链接文字打开；
        kb 优先用文档自身所属，页面级 kbId 兜底） */}
    <DocxOutlineModal
      open={outlineOpen}
