...
const BASE_METHODS: Array<Omit<MethodOption, 'reason'>> = [
  ...
  { value: 'hierarchical', label: '层级聚合切块', desc: '按章节自底向上聚合，块边界落在标题之间，块首自带标题链' },
...
const METHOD_ICONS: Record<string, React.ReactNode> = {
  ...
  hierarchical: <NodeIndexOutlined />,
  regex: <CodeOutlined />,
  qa: <MessageOutlined />,
...
const SmartParseWizard: React.FC<SmartParseWizardProps> = ({ open, doc, kbId, onCancel, onSuccess }) => {
  ...
  // 层级聚合切块候选：文档有标题结构时提供——Word + 结构解析的层级由 OOXML
  // 直读（精确）；PDF 等 OCR 类产物层级常丢失（全 ## 扁平），由后端用系统
  // 配置的「标题分层模型」重新分层后再聚合（未配置则回退规则并在日志留痕）
  const engineSuggested = analyze?.engine_suggestion?.suggested;
  const hasHeadings = !!analyze?.structure?.has_headings;
  const baseMethods = useMemo(
    () => ((hasHeadings || engineSuggested === 'docx_struct')
      ? BASE_METHODS
      : BASE_METHODS.filter(m => m.value !== 'hierarchical')),
    [hasHeadings, engineSuggested],
  );

  // Step2 选项：基础说明 + 画像推荐（主推荐标"推荐"+理由，agentic 标"可选"）
  const methodOptions = useMemo<MethodOption[]>(() => {
    if (!rec) return baseMethods.map(m => ({ ...m }));
