...
export const ragasMetricOptions = [
  {
    label: '忠实度',
    value: 'faithfulness',
    desc: '答案是否忠于检索到的上下文，无虚构（不依赖标准答案）',
  },
  {
    label: '答案相关性',
    value: 'answer_relevancy',
    desc: '答案与问题的相关程度（不依赖标准答案）',
  },
  {
    label: '上下文精确率',
    value: 'context_precision',
    desc: '检索上下文是否包含有用信息（不依赖标准答案）',
  },
  {
    label: '上下文召回率',
    value: 'context_recall',
    desc: '检索上下文是否覆盖必要信息（需 ground_truth）',
  },
  {
    label: '答案正确性',
    value: 'answer_correctness',
    desc: '答案与参考答案的匹配程度（需 ground_truth）',
  },
  {
    label: '答案相似度',
    value: 'answer_similarity',
    desc: '答案与参考答案的语义相似度（需 ground_truth）',
  },
];
...
export const overallHitRateOf = (quality: RetrievalQuality | null): number =>
...
/**
 * 统计分析共用数据源：自身统计（可选）+ RAGAS 任务 + 知识库列表 + 检索质量。
 * 自动初始化加载；提供发起评估后轮询任务列表（8s，直到无运行中任务）。
 */
export function useAnalyticsData(needStats: boolean) {
  const { message } = AntApp.useApp();
  const { user } = useAuth();
  // 检索质量选中的知识库记在 localStorage（按用户隔离），概览/详情跳转与刷新后都保持
  const qualityKbStorageKey = `analytics-quality-kb:${user?.id ?? 'anon'}`;
  const [stats, setStats] = useState<Stats | null>(null);
  const [ragas, setRagas] = useState<RagasStatus | null>(null);
  const [loading, setLoading] = useState(false);

  const [qKbs, setQKbs] = useState<KnowledgeBase[]>([]);
  const [qKbId, setQKbId] = useState<string | undefined>(() => {
    try {
      return window.localStorage.getItem(qualityKbStorageKey) ?? undefined;
    } catch {
      return undefined;
    }
  });
  ...
  // 自动恢复加载：只在挂载后为恢复的知识库加载一次（失败不重试死循环）
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [statsRes, ragasRes, kbsRes] = await Promise.all([
        needStats ? getStats() : Promise.resolve(null),
        getRagasStatus(),
        listKbs(),
      ]);
  ...
  const startPolling = useCallback(() => {
    ...
    pollTimerRef.current = window.setInterval(async () => {
      try {
        ...
        setRagas(res.data);
        const active = (res.data.tasks || []).some(
          (t) => t.status === 'queued' || t.status === 'pending' || t.status === 'running',
        );
  ...
  // 选择知识库 → 加载近 30 天检索质量统计；选择即持久化记忆
  const handleQualityKbChange = useCallback(
    async (id: string) => {
      setQKbId(id);
      setQuality(null);
      setQLoading(true);
      try {
        try {
          window.localStorage.setItem(qualityKbStorageKey, id);
        } catch {
    ...
    [qualityKbStorageKey, message],
  );

  // 初始恢复：记住了知识库且列表已加载 → 自动拉一次质量数据（同页返回/刷新均适用）
  useEffect(() => {
    if (qKbId && !quality && !qLoading && qKbs.some((k) => k.id === qKbId)) {
      if (autoLoadedRef.current === qKbId) return; // 加载失败不重试（防死循环），可手动重选
      autoLoadedRef.current = qKbId;
      void handleQualityKbChange(qKbId);
    }
  }, [qKbId, quality, qLoading, qKbs, handleQualityKbChange]);

  // 记住的知识库已被删除 → 清掉记忆恢复未选态
  useEffect(() => {
    if (qKbId && qKbs.length > 0 && !qKbs.some((k) => k.id === qKbId)) {
      setQKbId(undefined);
      autoLoadedRef.current = null;
      try {
        window.localStorage.removeItem(qualityKbStorageKey);
      } catch {
        // 同上静默
      }
    }
  }, [qKbId, qKbs, qualityKbStorageKey]);
...
/**
 * RAGAS 发起评估 / 测试集导入导出 / 聊天历史导入 / 任务报告 四个 Modal 的完整逻辑。
 * 返回触发函数（页面按钮/行点击使用）与需要渲染在页面底部的 Modals JSX。
 */
export function useRagasModals(opts: {
  user: { id: string; role?: string };
  kbOptions: { value: string; label: string }[];
  /** 任务状态变化（评估创建 / 任务取消）回调：页面刷新列表 + 轮询 + 新任务高亮 */
  onTasksChanged: (taskId: string) => void;
}) {
  const { message } = AntApp.useApp();
  // 报告 Modal
  ...
  const evalValidCount = useMemo(
    () =>
      (evalSamples || []).filter(
        (r: EvalSampleRow) => r?.question?.trim() && r?.ground_truth?.trim(),
      ).length,
    [evalSamples],
  );
  // 测试集文本导入/导出 Modal
  ...
  const exportSamplesText = (): string => {
    ...
    return (
      `# 每行一条：问题【Tab 键】正确答案（# 开头为注释行，导入时忽略）\n` +
  ...
  const parseTxtSamples = (
    ...
    for (let i = 0; i < lines.length && rows.length < 100; i++) {
      ...
      if (idx <= 0) {
        skipped++;
        invalid.push(`第 ${i + 1} 行：${t.slice(0, 30)}`);
        continue;
      }
  ...
  const parseExcelSamples = (
    ...
    let skipped = 0;
    const invalid: string[] = [];
    for (let i = 1; i < grid.length && rows.length < 100; i++) {
      // 跳过首行表头
      const cell = grid[i] || [];
  ...
  const handleStartEvaluation = async () => {
    ...
    try {
      ...
      message.success(
        `评估任务已创建：${res.data.name}（${res.data.sample_count} 条样本），运行中可查看进度`,
      );
      // —— 魔法注入动画：测试集注入任务列表（0.6s）→ 新任务行高亮 + 滚动 ——
      setEvalAnim(true); // 弹窗内测试集区域注入动画 + 魔法光晕
      opts.onTasksChanged(res.data.task_id); // 页面端：刷新列表 + 新行高亮 + 轮询
  ...
  return {
    ...
    openReport,
    /** 任务列表操作列构建（查看报告 / 取消） */
    makeColumns: () => makeRagasColumns({ user: opts.user, openReport, handleCancelTask }),
    modals: (
      <>
            ...
            <div className={`eval-inject-wrap${evalAnim ? ' eval-inject-anim' : ''}`}>
              ...
              <Space style={{ width: '100%', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <Space>
                  ...
                  <Button size="small" icon={<FileTextOutlined />} onClick={openTextModal}>
                    文本导入/导出
                  </Button>
                </Space>
              </Space>
                        ...
                        <Row key={field.key} gutter={8} align="top" style={{ marginBottom: 8 }}>
                          <Col flex="auto">
                            <Form.Item
                              ...
                              rules={[
                                { required: true, whitespace: true, message: '请输入测试问题' },
                              ]}
          ...
          <Space direction="vertical" style={{ width: '100%' }} size={12}>
            <Space wrap>
              ...
              <Segmented
                options={[
                  ...
                  { label: 'Excel(xlsx/csv)', value: 'excel' },
                ]}
                value={ioFormat}
              ...
              <Alert
                ...
                description={
                  <span style={{ fontSize: 12 }}>
                    ...
                    <br />
                    空行与 <Text code>#</Text> 注释行忽略；问题或答案为空的行跳过；最多 100 条。
                    <br />
                    <Text code>
                      在线式软地线和 IEC104 接地线配置有什么区别？&nbsp;&nbsp;两者不可同时配置
                    </Text>
                  </span>
                }
              />
            ...
            {fileParseResult ? (
              <Alert
                ...
                description={
                  fileParseResult.invalid.length > 0 ? (
                    <span style={{ fontSize: 12 }}>
                      无效内容（最多显示前 3 条）：
                      <br />
        ...
        >
          <Space direction="vertical" style={{ width: '100%' }} size={12}>
            ...
            <Space>
              <Button
                size="small"
                onClick={() => setChatPickSelected(chatPickList.map((s) => s.question))}
              >
                全选
              </Button>
        ...
        <AppModal
          ...
          title={
            <Space>
              <span>评估报告</span>
              {reportTask ? (
                <Tag color={statusOf(reportTask.status).color}>
                  {statusOf(reportTask.status).label}
                </Tag>
              ) : null}
            </Space>
          }
            ...
            <>
              ...
              >
                <AppTable
                  ...
                  columns={reportDetailColumns(report)}
...
/** 报告聚合评分卡片行 */
const ReportAggregate: React.FC<{ report: RagasReport }> = ({ report }) => {
...
/** 报告逐样本明细列（指标列从 aggregate.scores 动态生成） */
const reportDetailColumns = (report: RagasReport) => [
  { title: '#', dataIndex: 'idx', key: 'idx', width: 44 },
  {
    ...
    width: 220,
    render: (v: string) => (
      <Tooltip title={v}>{v.length > 60 ? v.slice(0, 60) + '...' : v}</Tooltip>
    ),
  },
  {
    title: '答案',
...
/** RAGAS 任务表格列（查看报告/取消 按钮） */
export const makeRagasColumns = (opts: {
  user: { id: string; role?: string };
  openReport: (t: RagasTask) => void;
  handleCancelTask: (t: RagasTask) => void;
}) => [
  {
    title: '任务名称',
    dataIndex: 'name',
  ...
  },
  {
    ...
    render: (v?: string) =>
      v === 'chat' ? (
        ...
        <Tag color="green">检索日志</Tag>
      ) : v === 'manual' ? (
        <Tag color="purple">手动填写</Tag>
      ) : (
        <Text type="secondary">—</Text>
      ),
  },
  ...
  },
  {
    ...
    render: (_: unknown, record: RagasTask) => {
      const canCancel =
        ...
        (record.user_id === opts.user.id || opts.user.role === 'super_admin');
      return (
        <>
          <Button
            size="small"
          ...
          {canCancel ? (
            <Popconfirm
              ...
              cancelText="保留"
              onConfirm={() => opts.handleCancelTask(record)}
            >
              <Button
                size="small"
                type="link"
...
/**
 * 每日检索量 / 命中率柱状图（柱高=当日检索量，颜色=当日命中率；无检索为灰底）。
 * barHeight 控制图区高度，概览页用紧凑值。
 */
export const DayRetrievalChart: React.FC<{
  daily: RetrievalQuality['daily'];
  barHeight?: number;
}> = ({ daily, barHeight = 96 }) => {
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: barHeight }}>
        {daily.map((d) => {
          const max = Math.max(...daily.map((x) => x.retrievals), 1);
          const h = d.retrievals > 0 ? Math.max(4, (d.retrievals / max) * (barHeight - 28)) : 4;
          const color =
            d.retrievals === 0
              ? '#94a3b8'
              : d.hit_rate >= 0.8
          ...
          return (
            ...
            >
              <Tooltip
                title={`${d.date}：${d.retrievals} 次检索，命中率 ${Math.round(d.hit_rate * 100)}%`}
              >
                <div
                  style={{
                    width: '72%',
              ...
              <Text
                type="secondary"
                style={{ fontSize: 9, lineHeight: '12px', whiteSpace: 'nowrap' }}
              >
                {d.date.slice(5)}
              </Text>
            </div>
...
/** 检索质量概览：知识库筛选 + 4 指标紧凑数字 + 文档命中排行 Top5（概览页右栏） */
export const QualitySummaryCard: React.FC<{
  qKbs: KnowledgeBase[];
  qKbId?: string;
  onKbChange: (id: string) => void;
  qLoading: boolean;
  quality: RetrievalQuality | null;
}> = ({ qKbs, qKbId, onKbChange, qLoading, quality }) => {
  const top5 = (quality?.hit_docs || []).slice(0, 5);
  const maxHits = Math.max(...top5.map((d) => d.hits), 1);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      ...
      {qLoading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, minHeight: 0, flex: 1 }}>
          ...
          <Row gutter={[8, 8]}>
            <Col span={6}>
              <div className="q-mini-stat q-mini-stat--col">
                <Text type="secondary" style={{ fontSize: 10 }}>
                  总检索次数
                </Text>
                <div style={{ fontSize: 17, fontWeight: 600 }}>{quality.total_retrievals}</div>
              </div>
            </Col>
            <Col span={6}>
              <div className="q-mini-stat q-mini-stat--col">
                ...
                <div style={{ fontSize: 17, fontWeight: 600 }}>
                  {quality.avg_hits_per_retrieval.toFixed(2)}
                </div>
              </div>
            </Col>
            <Col span={6}>
              <div className="q-mini-stat q-mini-stat--col">
                ...
                <div style={{ fontSize: 17, fontWeight: 600 }}>{Math.round(rate * 100)}%</div>
              </div>
            </Col>
            <Col span={6}>
              <div className="q-mini-stat q-mini-stat--col">
                ...
                <div style={{ fontSize: 17, fontWeight: 600 }}>{quality.zero_hit_docs.length}</div>
              </div>
            </Col>
          </Row>
          {/* 命中排行 Top5 */}
          <div style={{ minHeight: 0, flex: 1, overflow: 'hidden' }}>
            ...
            <div style={{ marginTop: 4, height: 'calc(100% - 22px)', overflowY: 'auto' }}>
              {top5.length ? (
                top5.map((d) => (
                  <div key={d.doc_id} style={{ marginBottom: 6 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                      <Tooltip title={d.doc_name}>
                        <Text
                          ellipsis
                          style={{ fontSize: 12, maxWidth: '70%', flex: 'none' }}
                        >
                          {d.doc_name}
                        </Text>
                      </Tooltip>
                    ...
                    <div className="q-hit-bar" style={{ width: `${(d.hits / maxHits) * 100}%` }} />
                  </div>
                ))
              ) : (
                <Text type="secondary" style={{ fontSize: 12 }}>
