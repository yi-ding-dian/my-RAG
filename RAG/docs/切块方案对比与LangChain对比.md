# 切块方案对比(七种方式 + 与 LangChain 对照)

> 本文档归纳知识库切块引擎的 7 种方式,并逐项与 LangChain 对应组件对比:
...
> `backend/normative/chunker.py`(hierarchical,独立包)、
...
| 层级聚合 hierarchical | `HierarchicalChunker`(backend/normative 包) | **自底向上**聚合章节树:先建树,再逐层问"整棵子树装得下吗"——装得下整棵成一个单元(可继续上浮与兄弟合并),装不下才在子节点之间拆;块边界永远落在标题之间,块首自带祖先标题链 | chapter_level(1)/chunk_size/overlap;PDF 另需「标题分层模型」还原层级 | 写作规范的层级文档(说明书/操作规程) | 边界(语义)与尺寸(预算)**兼得**;短小节并入父级不浪费;**依赖层级准确**;overlap 只作用于超长兜底二次切 |
...
| 层级聚合 hierarchical | **无直接对应**(最接近 `MarkdownHeaderTextSplitter`,但它只按 `#` 自顶向下切、把标题写进 metadata,既不聚合子树也不管块大小) | 都按标题组织块边界 | **纯自研**:自底向上的子树聚合(装得下整棵成块、可上浮)、块大小自动贴近预算、章级标题不可跨、块首祖先标题链;配套标题层级还原(编号规则 + LLM 混合分层)——均为 LC 无 |
...
5. **层级聚合切块(自底向上章节树聚合)**:先建章节树,再逐层判断"整棵子树能否装进一个块"——
   装得下整棵成一个单元并**可继续上浮**与兄弟合并,装不下才在子节点之间拆;于是块边界永远落在
   标题之间(**标题与正文永不被拆散**),块大小又自然贴近预算(**短小节并入父级,不浪费召回位**);
   章级标题为不可跨越的硬边界,块首自带祖先标题链。配套**标题层级还原**
   (`backend/normative/heading_llm.py`:带编号走规则、无编号交 LLM)——LC 无此算法。
6. **QA 问答对完整切块 + 入库占比规范性检测**(问答对占比 <50% → 阻止入库,带详情可强制继续)。
7. **Agentic LLM 语义切块 + 5 类标签 + 双挡成本护栏**(>5 万字拒绝、1-5 万字需确认)+ 失败回退 title。
8. **块级全局偏移 char_start/char_end**,且保证 `text == full[char_start:char_end]` 切片一致性(溯源/引用/图谱实体定位全靠它;LC 默认不感知全文坐标)。
9. **Excel 行窗口分段**(700 字符/段、每段重复表头 + 标题带"第 x-y 行",与公式引擎/类 Excel 预览协同)——LC 无此概念。
10. 全链路零第三方切块依赖(私有化部署可控)。
...
|---|---|
| 快速起步、组件丰富 | 起步期功能可枚举(切块+检索+流式);自研 7 种切法已覆盖,且每个都能讲清为什么 |
...
> 7 种切块方式中,naive/title/parent 与 LangChain 思想同源;句子感知、保护区间、标题智能回退、QA 对、Agentic 语义切、块级偏移、层级聚合这 7 样是**纯自研、LC 没有**的——通用方案做到了 LC 的 80%,差异化的 20% 才是检索质量的关键。
